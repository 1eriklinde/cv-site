...
pdf-manifest.json  fingerprints pairing the CV's text with the printed PDF
...
npm run pdf:stamp record that the PDF matches the CV (run after reprinting it)
...
- `public/erik-linde-cv.pdf` is generated by printing this site to PDF, so it
  ...
  Regenerate after content changes: print the page to PDF from the browser, then
  `npm run pdf:stamp`. The test suite fails if the CV's printed text changes
  without the PDF being reprinted, or if the PDF changes without being stamped —
  nothing else in the pipeline can notice a stale PDF, because nothing else can
  read one.
